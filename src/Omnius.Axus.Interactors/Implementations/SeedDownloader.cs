...
public sealed partial class SeedDownloader : AsyncDisposableBase, ISeedDownloader
{
    ...
    private readonly SeedDownloaderRepository _seedDownloaderRepo;
    ...
    private SeedDownloader(IProfileDownloader profileDownloader, IAxusServiceMediator serviceMediator, ISingleValueStorageFactory singleValueStorageFactory, IKeyValueStorageFactory keyValueStorageFactory, IBytesPool bytesPool, SeedDownloaderOptions options)
    {
        ...
        _seedDownloaderRepo = new SeedDownloaderRepository(Path.Combine(_options.ConfigDirectoryPath, "status"));
    ...
    {
        await _seedDownloaderRepo.MigrateAsync(cancellationToken);
    ...
    private async Task WatchLoopAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            for (; ; )
            {
                ...
                await this.SyncSeedDownloaderRepo(signatures, cancellationToken);
    ...
    private async ValueTask SyncSeedDownloaderRepo(ImmutableHashSet<OmniSignature> targetSignatures, CancellationToken cancellationToken = default)
    {
        using (await _asyncLock.LockAsync(cancellationToken))
        {
            foreach (var profileItem in _seedDownloaderRepo.BarkItems.FindAll())
            {
                if (targetSignatures.Contains(profileItem.Signature)) continue;
                _seedDownloaderRepo.BarkItems.Delete(profileItem.Signature);
            }

            foreach (var signature in targetSignatures)
            {
                if (_seedDownloaderRepo.BarkItems.Exists(signature)) continue;
                ...
                _seedDownloaderRepo.BarkItems.Upsert(newBarkItem);
            }
        }
    }
    ...
    private async ValueTask SyncSubscribedShouts(CancellationToken cancellationToken = default)
    {
        using (await _asyncLock.LockAsync(cancellationToken))
        {
            ...
            foreach (var barkItem in _seedDownloaderRepo.BarkItems.FindAll())
            {
    ...
    private async ValueTask SyncSubscribedFiles(CancellationToken cancellationToken = default)
    {
        using (await _asyncLock.LockAsync(cancellationToken))
        {
            ...
            {
                if (_seedDownloaderRepo.BarkItems.Exists(rootHash)) continue;
                await _serviceMediator.UnpublishFileFromMemoryAsync(rootHash, Zone, cancellationToken);
            }

            foreach (var rootHash in _seedDownloaderRepo.BarkItems.FindAll().Select(n => n.RootHash))
            {
    ...
    private async ValueTask<CachedBarkContent?> TryInternalExportAsync(OmniSignature signature, DateTime cachedContentShoutUpdatedTime, CancellationToken cancellationToken = default)
    {
        using (await _asyncLock.LockAsync(cancellationToken))
        {
            ...
            var barkItem = _seedDownloaderRepo.BarkItems.FindOne(signature);
    ...
    public async ValueTask<SeedDownloaderConfig> GetConfigAsync(CancellationToken cancellationToken = default)
    {
        using (await _asyncLock.LockAsync(cancellationToken))
        {
            var config = await _configStorage.TryGetValueAsync<SeedDownloaderConfig>(cancellationToken);

            if (config is null)
            {
                config = new SeedDownloaderConfig(
