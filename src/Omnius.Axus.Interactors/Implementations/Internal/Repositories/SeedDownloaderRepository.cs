...
internal sealed class SeedDownloaderRepository : DisposableBase
{
    private readonly LiteDatabase _database;

    public SeedDownloaderRepository(string dirPath)
    {
    ...
    public sealed class SubscribedSeedItemRepository
    {
        ...
        private ILiteCollection<DownloadingSeedItemEntity> GetCollection()
        {
            var col = _database.GetCollection<DownloadingSeedItemEntity>(CollectionName);
            return col;
        }
        ...
        public IEnumerable<DownloadingSeedItem> FindAll()
        {
            lock (_lockObject)
            {
                var col = this.GetCollection();
        ...
        public DownloadingSeedItem? FindOne(OmniSignature signature)
        {
            lock (_lockObject)
            {
                var signatureEntity = OmniSignatureEntity.Import(signature);
        ...
        public void Upsert(DownloadingSeedItem item)
        {
            lock (_lockObject)
            {
                var itemEntity = DownloadingSeedItemEntity.Import(item);
