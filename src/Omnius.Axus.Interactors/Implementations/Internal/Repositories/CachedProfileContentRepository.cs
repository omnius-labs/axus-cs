...
internal sealed class CachedProfileContentRepository : DisposableBase
{
    ...
    public CachedProfileContentRepository(string dirPath, IKeyValueStorageFactory keyValueStorageFactory, IBytesPool bytesPool)
    {
    ...
    public async ValueTask UpsertAsync(CachedProfileContent content, CancellationToken cancellationToken = default)
    {
        await _storage.WriteAsync(content.Signature.ToString(), content, cancellationToken);
    }

    public async ValueTask<CachedProfileContent?> TryReadAsync(OmniSignature signature, CancellationToken cancellationToken = default)
    {
        return await _storage.TryReadAsync<CachedProfileContent>(signature.ToString(), cancellationToken);
    }
