...
internal sealed class BarkUploaderRepository : DisposableBase
{
    private readonly LiteDatabase _database;

    public BarkUploaderRepository(string dirPath)
    {
    ...
    public sealed class PublishedBarkItemRepository
    {
        ...
        private ILiteCollection<UploadingBarkItemEntity> GetCollection()
        {
            var col = _database.GetCollection<UploadingBarkItemEntity>(CollectionName);
            return col;
        }
        ...
        public IEnumerable<UploadingBarkItem> FindAll()
        {
            lock (_lockObject)
            {
                var col = this.GetCollection();
        ...
        public UploadingBarkItem? FindOne(OmniSignature signature)
        {
            lock (_lockObject)
            {
                var signatureEntity = OmniSignatureEntity.Import(signature);
        ...
        public void Upsert(UploadingBarkItem item)
        {
            lock (_lockObject)
            {
                var itemEntity = UploadingBarkItemEntity.Import(item);
